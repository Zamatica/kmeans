...
class Color:
    ...
    def distance(self, red, green, blue) -> int:
        return (self.red - red) ** 2 + \
               (self.green - green) ** 2 + \
               (self.blue - blue) ** 2

    def distance_pixel(self, pixel) -> int:
        return (self.red - pixel[2]) ** 2 + \
    ...
    def from_hex_index(self, index):
        hex_string = COLORS[index]
        self.from_hex(hex_string)

    def from_hex(self, string):
        self.red, self.green,  self.blue = tuple(int(string[i:i+2], 16) for i in (1, 3, 5))
